...
GraphTAD *init_city(int places,int roads,int *src,int *dest) {
    GraphTAD *graph = graph_create(places + 1);
    ...
    for(i = 0; i < roads; i++) {
        ...
        graph_set_node_link(graph,from,to,dist);
    }
...
int min_path_cost(GraphTAD *graph,int places,int src,int dest) {
    if(places <= 0) return -1;

    HashSet *discovered = hashset_create();
    PQ *frontier = initQueue(places);

    int i;
    for(i = 1; i <= places; i++) {
        graph_set_node_value(graph,i,ARC_INF);  //all nodes starts as unreachable
        enqueue(GetHeapNode(ARC_INF,i),frontier);
    }

    updateQueueElement(0,src,frontier);
    graph_set_node_value(graph,src,0);

    unsigned int cost = ARC_INF;

    while(!isQueueEmpty(frontier)) {
        heapNode step = dequeue(frontier);
        hashset_insert(discovered,step.data.place_id);

        if(step.data.place_id == dest) {
            //reached destination, minimal cost is held by place_dist
            cost = step.data.place_dist;
            break;
        }
        ...
        for(i = 0; i < graph->nodes[step.data.place_id]->unique_links_len; i++) {
            ...
            if(!hashset_contains(discovered,neighbor,NULL)) {
                ...
                if(graph_get_node_value(graph->nodes[step.data.place_id]->unique_links[i]->node) > new_cost) {
    ...
    return((cost >= ARC_INF) ? -1 : cost);
}

void instantiate_city(int places,int roads) {
    int src,dest;

    GraphTAD *graph = init_city(places,roads,&src,&dest);
    printf("%d\n",min_path_cost(graph,places,src,dest));
    graph_destroy(graph);
}
...
int main() {
    ...
    while(1) {
        ...
        if(places <= 0 && roads <= 0) break;
        instantiate_city(places,roads);
    }
